...
def get_file_to_write_results(spider_class_name):
    out_file_name = spider_class_name + '_result.txt'

    store_folder_path = os.path.join(get_base_dir_path(), spider_class_name)
    if not os.path.exists(store_folder_path):
        os.makedirs(store_folder_path)

    return os.path.join(store_folder_path, out_file_name)

def get_base_dir_path():
    path = os.path.join(os.sep.join(sys.path[0].split(os.sep)[:-1]), 'tests_local')
